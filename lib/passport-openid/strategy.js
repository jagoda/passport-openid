...
function Strategy(options, verify) {
  ...
  if (options.profile) {
    ...
    if (options.ax) {
      ...
      attributes = {};
      mappings = {};
      options.ax.forEach(function (attribute) {
        attributes[attribute.type] = attribute.required ? "required" : "optional";
        mappings[attribute.type] = attribute.alias;
        self._ax.push(attribute.alias);
      });
    }
    
    var ax = new openid.AttributeExchange(attributes, mappings);
    extensions.push(ax);
  }
...
Strategy.prototype._parseProfileExt = function(params) {
  ...
  if (this._ax) {
    for (var key in params) {
      if (!profile[key] && this._ax.indexOf(key) !== -1) {
